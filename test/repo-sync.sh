...
readonly STABLE_REPO_URL=https://storage.googleapis.com/storageos-charts
readonly GCS_BUCKET_STABLE=gs://storageos-charts
...
setup_helm_client() {
    ...
    helm repo add storageos "$STABLE_REPO_URL"
}

authenticate() {
    echo "Authenticating with Google Cloud..."
    gcloud auth activate-service-account --key-file <(base64 -d <<< "$SYNC_CREDS")
}
